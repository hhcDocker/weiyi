...
class WxPayApi
{
    /**
     *
     * 企业付款,WxPayMerchantOrder中partner_trade_no、openid、check_name、amount必填
     * @param MerchantOrder $input
     */
    public static function merchantOrder($inputObj, $timeOut = 6)
    {
        ...
        $result = WxMchPayResults::Init($response);
     ...
     * @param MerchantOrder $input
     */
    public static function merchantQuery($inputObj, $timeOut = 6)
    {
        $url = "https://api.mch.weixin.qq.com/mmpaymkttransfers/gettransferinfo";
        //检测必填参数
        if(!$inputObj->IsPartner_trade_noSet()) {
            throw new WxPayException("缺少查询企业付款接口必填参数partner_trade_no！");
        }
    
         
        $inputObj->SetMcd_appid(WxPayConfig::APPID);//公众账号ID
        $inputObj->SetMchid(WxPayConfig::MCHID);//商户号
        $inputObj->SetNonce_str(self::getNonceStr());//随机字符串
