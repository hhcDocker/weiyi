...
class WtService extends APIAuthController
{
        ...
        if (strpos($full_url,'tmall.com')) { //天猫
            ...
            if (preg_match('/shop[.\d\w]+.taobao.com/',$full_url)){ //店铺链接
                if (strpos($full_url, 'shop.m.taobao.com')) { //淘宝店铺移动端1
                    ...
                    if (empty($m) || !isset($m[1])) {
                        preg_match('/shop_id=(\d+)/',$full_url,$m);
                        if (empty($m) || !isset($m[1])) {
                            throw new APIException(30001, ['url' => $full_url]);
                        }else{
                            $shop_id = $m[1];
                            //根据shopid查表
                            $shop_url='http://shop.m.taobao.com/shop/shop_index.htm?shop_id='.$shop_id;
                            $shop_info = model('AliShops')->getShopInfoByShopId($shop_id);
                        }
                    }else{
                        $user_id =end($m);
    ...
    /**
     * [getRemainExperienceTime description]
     * @return [type] [description]
     */
    public function getRemainExperienceTime()
    {
        ...
        if (strpos($shop_url, 'tmall.com/shop') || preg_match('/\w+[.\w]+tmall.com/',$shop_url)){ //天猫店铺
            ...
            if (empty($m) || !isset($m[1])){
                ...
                if (empty($m) || !isset($m[1])) {
                    ...
                    $shop_id = $m[1];
                    //根据shopid查表
                    $ali_shop_url='https://shop.m.taobao.com/shop/shop_index.htm?shop_id='.$user_id;
                    $shop_info = model('AliShops')->getShopInfoByShopId($shop_id);
                }
            }else{
                $user_id =end($m);
        ...
        if (empty($shop_info)) { //表示从来没抓取过该店铺数据，或者信息不全
            ...
            if (!empty($service_info) && ($service_info['service_end_time'] - $service_info['service_start_time'] >=365*24*60*60-1)){
        ...
        if (strpos($shop_url, 'tmall.com/shop') || preg_match('/\w+[.\w]+tmall.com/',$shop_url)){ //天猫店铺
            ...
            if (empty($m) || !isset($m[1])){
                ...
                if (empty($m) || !isset($m[1])) {
                    ...
                    $shop_id = $m[1];
                    $ali_shop_url = 'http://shop.m.taobao.com/shop/shop_index.htm?shop_id='.$shop_id;
                    //根据shopid查表
                    $shop_info = model('AliShops')->getShopInfoByShopId($shop_id);
                }
            }else{
    ...
    public function getTransRecords()
    {
        ...
        if (!empty($record_list)) {
            foreach ($record_list as $k => $v) {
                ...
                //状态
                if ($v['service_end_time'] - $v['service_start_time'] <= 365*24*60*60-1) { //未支付
                    $record_list[$k]['service_type'] = 1;
                }elseif ($v['service_end_time'] > time() && ($v['service_end_time'] - $v['service_start_time'] > 365*24*60*60-1)){ //生效中
                    $record_list[$k]['service_type'] = 2;

                }elseif ($v['service_end_time'] <= time() && ($v['service_end_time'] - $v['service_start_time'] > 365*24*60*60-1)) { //已到期
    ...
    private function manageServiceInfo($service_info=array(),$qrcode_url='',$wj_shop_id=0,$shop_url='',$ali_shop_id=0)
    {
        ...
        if ($service_info['service_end_time'] - $service_info['service_start_time'] <= 365*24*60*60-1) { //体验期，包括新转化情况
