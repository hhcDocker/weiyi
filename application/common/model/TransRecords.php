...
class TransRecords extends Base
{
    ...
    public function getRecordsByManagerId($manager_id=0,$pageIndex=1, $pageSize=10,$service_type=0)
    {
        ...
        if ($service_type==1) { //未支付
            $where ='s.service_end_time-s.service_start_time<=365*24*60*60-1';
        }elseif ($service_type==2) { //生效中
            $where ='s.service_start_time<UNIX_TIMESTAMP(NOW()) and s.service_end_time>unix_timestamp(now()) and s.service_end_time-s.service_start_time>365*24*60*60-1';
        }elseif ($service_type==3) { //已到期
            $where ='s.service_start_time>UNIX_TIMESTAMP(NOW()) or s.service_end_time<unix_timestamp(now()) and s.service_end_time-s.service_start_time>365*24*60*60-1';
        }
